...
void ValuesDiffusion::average(void){
...
public
     const ValueDiffusion* v;


//    void allocate_values();
//    void deallocate_values();
      void  output(ValuesDiffusion v, int t)
...
//    double sum = 0, mmm = f_[0];
     ...
     ofs << "#t=" << std::setw(8) << t << "    x,    f";
    for(int i=0; i<defines::nx; i++) {
     ofs << std::setw(8) << x__(i) << " "
              << std::setw(8) << f__[i] << std::endl;
}
}

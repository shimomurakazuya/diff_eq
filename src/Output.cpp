...
OutputDiffusionData(const ValuesDiffusion& v, const int t) {
    ...
    for(int j=0; j<defines::ny; j+=defines::downsize) {
        for(int i=0; i<defines::nx; i+= defines::downsize) {
            ...
            //f_a = defines::fmax * cos(v.xx()[i]/defines::lx*2.0*M_PI ) * exp(- defines::c_dif* ((2.0 * M_PI/defines::lx)* (2.0 * M_PI/defines::lx)) * double(t)* defines::dt *defines::iout  );
            diff = f_ana - v.ff()[ji];
            fprintf(fp2,"%8.3f %8.3f %8.8f %8.3f %8.3f %10.8e  %10.8e \n",v.xx()[i],v.yy()[j], v.ff()[ji], f_ave, f_max,f_ana, diff ); 
        }
        fprintf(fp2,"\n" ); 
    }    
...
print_elapsetime(const double st_time,const double ed_time, const double* ave_time){
    real elapse_time, bw;
    real cal_ratio, gf;
    real ave_time_min=10;
    
    for(int i=0; i<defines::iter; i++) {
    ave_time_min = std::fmin(ave_time_min, ave_time[i]);
    }

    elapse_time = ed_time - st_time;
    bw = 1e-09* defines::data_num * sizeof(real) * defines::ncell / ave_time_min;
    gf = 1e-09* defines::flop_num * defines::ncell / ave_time_min;    
...
#else
    char filename[1024];
    sprintf(filename, "data/log_CPU%04d.dat", defines::thread_num ) ;
    FILE* fp = fopen( filename ,"w");
#endif
    ...
    fprintf(fp, "elapse time = %lf, elapse time loop min= %lf,\n", elapse_time, ave_time_min);
    fprintf(fp, "OMP_thread_num = %d, peak Flops(loof_line) = %lf, \n", defines::thread_num, defines::loof_line);
